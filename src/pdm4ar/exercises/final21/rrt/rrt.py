...
from pdm4ar.exercises.final21.rrt.motion_primitives import MotionPrimitives
...
class RRT:
    def __init__(self,
        ...
        self.dt = dt
        self.motion_primitives = MotionPrimitives(self.sg, 1)
    ...
    def _update_graph(self, x_idx):
        ...
        if np.linalg.norm(x_nearest.pos - x_rand) > self.radius:
            return
        self.motion_primitives.generate(x_nearest.state,
                                        MOTION_PRIMITIVE_INPUT_DIVISIONS)
        trajectory, end = self.motion_primitives.closest(x_rand)
        x_new_dist = self.distance.dist(end, x_rand)
        if x_new_dist > STEERING_MAX_DIST:
        ...
        near_nodes = self._near_on_tree(
            x_new, self.radius,
            lambda n1, n2: self.distance.dist(n1.pos, n2.pos))

        self.tree.add_node(x_new)
        print(f"added node {x_new}")
        ...
        for x_near in near_nodes:
            collision_free = True
            line_cost = self.distance.dist(x_near.pos, x_new.pos)
        ...
        for x_near in near_nodes:
            ...
            collision_free = True
            line_cost = c_min + self.distance.dist(x_new.pos, x_near.pos)
            if line_cost < x_near.cost:
                x_parent = self.tree.predecessors(x_near)
                self.tree.remove_edge(x_parent, x_near)
                self.tree.add_edge(x_new, x_near)
